...
# =====================================================
# 3. CSS GIAO DIỆN (ĐÃ ĐỘ LẠI MODAL RA GIỮA)
# =====================================================
st.markdown("""
<style>
    ...
    /* --- QUAN TRỌNG: BIẾN FORM ĐĂNG NHẬP THÀNH POPUP GIỮA MÀN HÌNH --- */
    div[data-testid="stForm"] {
        ...
        border: 2px solid #0f988b !important;
    }
    ...
    .limit-icon { font-size: 60px; display: block; margin-bottom: 10px; text-align: center; }
    .limit-title { font-size: 22px; font-weight: bold; color: #00796b; margin-bottom: 10px; text-align: center; }
    .limit-desc { font-size: 15px; color: #555; margin-bottom: 20px; text-align: center; line-height: 1.5; }
    ...
    .zalo-btn { display: flex !important; align-items: center; justify-content: center; width: 100%; background-color: #0f988b; color: white !important; border: none; border-radius: 25px; font-weight: bold; font-size: 15px; height: 45px !important; text-decoration: none !important; margin: 10px 0 !important; box-shadow: 0 4px 10px rgba(15, 152, 139, 0.3); }
    div[data-testid="stForm"] button { height: 45px !important; border-radius: 8px !important; font-weight: 500 !important; color: #31333F !important; width: 100%; }
...
if db is None or model is None:
    st.warning("🧘‍♂️ Đang khởi động não bộ... Vui lòng chờ chút rồi F5 nhé.")
    st.stop()
...
# =====================================================
# 6. FORM ĐĂNG NHẬP (MODAL Ở GIỮA)
# =====================================================
def render_login_form(show_limit_msg=False):
    with st.form("login_form"):
        ...
        if show_limit_msg:
            ...
            <div style="text-align: center;">
                ...
                <div class="limit-title">Đã đạt giới hạn!</div>
                <div class="limit-desc">
                    Hệ thống nhận thấy bạn đã dùng hết lượt dùng thử.<br>
                    Để tiếp tục tra cứu <b>Kho dữ liệu 15 triệu từ</b> và nhận ưu đãi <b>Mua Thảm tặng Tài khoản Member</b>, mời bạn liên hệ Admin:
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown("<div style='text-align:center; font-size:12px; color:#888; margin-bottom:15px'>(Hoặc đăng nhập nếu bạn đã là Member ở phía dưới)</div>", unsafe_allow_html=True)
        else:
            st.markdown("### 🔐 Đăng nhập Member")
...
# --- XỬ LÝ HẾT LƯỢT (HIỆN MODAL GIỮA MÀN HÌNH) ---
if not can_chat:
    st.markdown("""<style>div[data-testid="stChatInput"] {display: none !important;}</style>""", unsafe_allow_html=True)
    # Lớp nền mờ
    st.markdown('<div class="modal-backdrop"></div>', unsafe_allow_html=True)
    # Form đăng nhập (CSS đã ép ra giữa)
    render_login_form(show_limit_msg=True)
    st.stop()
...
if prompt := st.chat_input("Hỏi tôi về Yoga..."):
    ...
    with st.chat_message("assistant"):
        if db:
            ...
            # PROMPT ĐÃ CHỈNH: KHÔNG VIẾT HOA, KHÔNG HEADER TO
            sys_prompt = f"""
...
if not st.session_state.authenticated and can_chat:
    ...
    with st.expander("🔐 Đăng nhập (Dành cho Member)", expanded=False):
        render_login_form(show_limit_msg=False)
