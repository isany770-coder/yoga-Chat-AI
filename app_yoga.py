...
# =====================================================
# 4. KẾT NỐI API & DRIVE (BẢN ỔN ĐỊNH)
# =====================================================
...
if db is None or model is None:
    st.warning("🧘‍♂️ Hệ thống đang khởi động, vui lòng vuốt xuống để tải lại (F5)...")
    st.stop()
...
if not can_chat:
    ...
    st.markdown(f"""<div class="limit-container"><div style="font-size:50px;margin-bottom:10px">🧘‍♀️</div><div style="font-size:22px;font-weight:bold;color:#0f988b;margin-bottom:10px">Đã đạt giới hạn!</div><p style="color:#555;margin-bottom:20px">Bạn đã dùng hết {TRIAL_LIMIT} lượt hỏi thử.<br>Vui lòng đăng nhập.</p></div><br>""", unsafe_allow_html=True)
    c1, c2, c3 = st.columns([1,2,1])
    with c2: render_login_form()
    st.stop()

if prompt := st.chat_input("Hỏi tôi về Yoga..."):
    db_data[user_key]["count"] += 1; db_data[user_key]["history"].append({"role": "user", "content": prompt}); save_data(db_data)
    st.session_state.messages.append({"role": "user", "content": prompt})
    with st.chat_message("user"): st.markdown(prompt)

    with st.chat_message("assistant"):
        if db:
            # Lấy hẳn 15 kết quả để vớt Science sâu hơn
            docs = db.similarity_search(prompt, k=15)
            
            # --- LOGIC LỌC MỚI: ƯU TIÊN TUYỆT ĐỐI SCIENCE ---
            ...
            # Lấy TẤT CẢ Science tìm được (không giới hạn), rồi mới bù thêm QA/Blog
            ...
            for i, d in enumerate(final_docs):
                ...
                # Logic hiển thị Link: Chỉ lấy link xịn (>10 ký tự)
            ...
            if solutions:
                ...
                sol_context = f"\nLưu ý: Cuối bài, hãy khuyên dùng: {names}."

            # --- PROMPT MỚI: ÉP VIẾT CHỮ THƯỜNG ---
            ...
            1. **KHÔNG VIẾT HOA TOÀN BỘ TIÊU ĐỀ**. Hãy dùng kiểu chữ thường (Sentence case). Ví dụ: Thay vì "KẾT LUẬN", hãy viết "Kết luận". Thay vì "KHOA HỌC...", hãy viết "Góc nhìn khoa học...".
            2. Nội dung ngắn gọn, súc tích.
            3. {sci_instruct}
            4. Luôn nhắc nhở an toàn (Ahimsa).
            ...
            try:
                ...
                # HIỂN THỊ LINK (Đảm bảo Science lên đầu)
                if source_map:
                    # Sắp xếp lại: Science -> QA -> Blog
