...
OUTPUT_ZIP = "/tmp/brain_v10_score.zip"
EXTRACT_PATH = "/tmp/brain_v10_score"
...
if db is None or model is None:
    st.warning("🧘‍♂️ Đang khởi động não bộ V10... Vui lòng F5 nếu chờ quá 1 phút.")
    st.stop()
...
if prompt := st.chat_input("Hỏi tôi về Yoga..."):
    ...
    with st.chat_message("assistant"):
        if db:
            # 1. VÉT 80 KẾT QUẢ THÔ
            docs = db.similarity_search(prompt, k=80)
            
            # 2. HỆ THỐNG CHẤM ĐIỂM "UY TÍN" (SCORING SYSTEM)
            ...
            for d in docs:
                ...
                # a. Điểm Uy Tín (Loại bài)
                if dtype == 'science': score += 100    # Nghiên cứu ưu tiên số 1
                elif dtype == 'qa': score += 50        # Chuyên gia ưu tiên số 2
                
                # b. Điểm Liên Quan (Keyword khớp tiêu đề)
            ...
            final_docs = [x[1] for x in ranked_docs[:6]]

            # 4. TẠO CONTEXT VỚI ID ĐỊNH DANH [1], [2]...
            context_parts = []
            source_map = {}
            source_index = 1
            ...
            for d in final_docs:
                ...
                # Nhãn hiển thị cho AI đọc
                label = "BLOG"
                if dtype == 'science': label = "KHOA HỌC/NGHIÊN CỨU"
                elif dtype == 'qa': label = "CHUYÊN GIA"
                
                # Gán ID [1], [2]...
                context_parts.append(f"NGUỒN [{source_index}] (Loại: {label}):\nTiêu đề: {title}\nNội dung: {d.page_content}")
                
                if url and len(str(url)) > 10:
                    source_map[source_index] = {"title": title, "type": dtype, "url": url}
                    source_index += 1
            ...
            # 5. PROMPT "KHẮC NGHIỆT" (V10)
            sys_prompt = f"""
            Bạn là Chuyên gia Yoga Khoa học & Trị liệu.
            DỮ LIỆU THAM KHẢO (Đã đánh số [1], [2]...):
            {full_context}
            {sol_context}

            QUY TẮC BẮT BUỘC:
            1. KHÔNG VIẾT HOA TOÀN BỘ TIÊU ĐỀ. Dùng chữ thường (Sentence case).
            2. Mọi thông tin đưa ra phải có dẫn chứng bằng cách gắn số ID nguồn tương ứng. 
               Ví dụ: "Yoga giúp giảm đau lưng [1], nhưng cần tập đúng [3]."
            3. Ưu tiên tuyệt đối thông tin từ nguồn [KHOA HỌC/NGHIÊN CỨU].
            4. Nếu không có thông tin trong Dữ liệu, hãy nói "Hiện chưa có dữ liệu cụ thể trong kho tàng thư về vấn đề này". Đừng bịa đặt.
            ...
            try:
                ...
                # Render Nguồn (Theo ID [1], [2]...)
                if source_map:
                    links_html = "<div class='source-box'><strong>📚 Nguồn kiểm chứng:</strong><div style='margin-top:8px'>"
                    for idx, info in source_map.items():
                        ...
                        links_html += f"<div style='margin-bottom:6px'><strong>[{idx}]</strong> {tag_html} <a href='{info['url']}' target='_blank' style='text-decoration:none; color:#0f988b; font-weight:500'>{info['title']}</a></div>"
                    links_html += "</div></div>"
